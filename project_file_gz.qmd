...
goalkeeper_stats("https://www.premierleague.com/stats/top/players/", "yellow_card")
```
...
```{r}
scrape_events("https://www.penguinrandomhouse.com/authors/events")
```

Penguin Random House Table Scraping 

```{r}
robotstxt::paths_allowed("https://jobsearch.createyourowncareer.com/PRH_US/go/PRH_US_Publishing_and_Corporate/9050001/?locale=en_US")
session <- bow("https://jobsearch.createyourowncareer.com/PRH_US/go/PRH_US_Publishing_and_Corporate/9050001/?locale=en_US", force = TRUE)

result <- scrape(session) |>
  html_nodes(css = "table") |> 
  html_table(header = TRUE, fill = TRUE)

careers_data <- result [[1]] |> 
  clean_names()
```
